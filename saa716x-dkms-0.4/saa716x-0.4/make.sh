...
if [ -z "$clean" ]; then
    if [ -z "$batch" ] && [ -f /tmp/$archive.zip -o -d $source/$archive ]; then
...
#        rm -f  /tmp/$archive.zip >/dev/null 2>&1
...
if [ ! -f /tmp/$archive.zip ]; then
    print "Downloading saa716x-$release.zip"
    wget -O /tmp/$archive.zip https://github.com/s-moch/linux-saa716x/archive/$archive.zip || print "Download failed" 1
...
# move /tmp/saa716x.log /usr/share/doc/saa716x-dkms/saa716x-dkms.log-$(date +%Y.%M.%dT%H:%M:%S)
mv /tmp/saa716x.log /usr/share/doc/saa716x-dkms/saa716x-dkms.${kernel}_$(date +%Y.%m.%dT%H:%M:%S).log
